export default class {
  constructor(option) {
    ...
    this.settings = {
      ...
      extraCss: '', // 额外的css逗号分隔
      popTitle: '', // 标题
      beginCallback: null, // 打开之前的回调函数
      endCallback: null, // 成功打开后的回调函数
      defaultCallback: null, // 成功打开后的默认回调函数
      ids: '' // 局部打印的id
    };
  ...
  async init() {
    await new Promise((resolve, reject) => {
      let res = this.settings.beginCallback();
      // 等待200mm视图更新后再打开预览窗口
      setTimeout(() => {
        resolve(res)
        reject(res)
