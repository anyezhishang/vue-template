    ...
    const localPrint = () => {
      if (closeBtn) {
        ...
        new Print({
          ...
          beginCallback() { // 调用成功打开之前的回调事件
            binding.value.beginCallback && binding.value.beginCallback.call(vue, binding)
          },
          endCallback() { // 调用成功打开之后的回调事件
            // 防止多次点击打印(点击按钮时就执行了这函数，把closeBtn改为了true，没用)
            closeBtn = true;
            // 改变传入的方法的this重新指向vue，并传出打印对象
            binding.value.endCallback && binding.value.endCallback.call(vue, binding)
          },
          afterCallback() { // 调用打印之后的回调事件
            binding.value.afterCallback && binding.value.afterCallback.call(vue, binding)
          },
        });
      }
    };
